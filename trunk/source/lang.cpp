...
void printKey(TCHAR *s, ACCEL *a)
{
	*s=0;
	if(a->key==0) return;
	if(a->fVirt&FCONTROL) lstrcat(s, lng(667, _T("Ctrl+")));
	if(a->fVirt&FSHIFT) lstrcat(s, lng(668, _T("Shift+")));
	if(a->fVirt&FALT) lstrcat(s, lng(669, _T("Alt+")));
	s+=lstrlen(s);
...
static void loadLang()
{
	...
	lstrcpy(cutPath(buf), _T("language\\"));
