/*
	structures Out_Module and In_Module are from Winamp SDK
	we don't use all methods
	*/
...
public:
	static In_Module *in;
	static int lock;
	static TCHAR menuName[128];

	static void loadPlugins();
	static void unloadPlugins();
	static bool open(Titem *item, int action);
	static bool run();
	static bool stop();
	static bool pause();
	static bool seek(LONGLONG pos);
	static bool getDuration(LONGLONG *duration);
	static REFERENCE_TIME streamTime();
};
